...
package capstonewar.pages.Promo;

import admin.entity.Promo;
import admin.session.PromoFacadeLocal;
 ...
 *
 * @version PromoMaintenance.java
 * @version Created on Oct 8, 2017, 10:39:36 PM
 * @author Jefferson Van
 */

public class PromoMaintenance extends AbstractPageBean {
    @EJB
    ...
    public void init() {
        ...
        try {
            ...
            log("PromoMaintenance Initialization Failure", e);
    ...
    public void prerender() {
        ...
        Promo[] proArray = sb1.getPromoData();
        if(proArray==null){
            sb1.setPromoData(new Promo[0]);
        }
    }
    ...
    public void all_processValueChange(ValueChangeEvent vce) {
        ...
        if(all.isChecked()){
            List<Promo> caseList = promoFacade.findAll();
            Promo[] proArray = caseList.toArray(new Promo[0]);
            sb1.setPromoData(proArray);
            }
            else{
                sb1.setPromoData(null);
                return;
            }
    }
    ...
    public String btnSearch_action() {
        ...
        boolean checkIfExists = promoFacade.findThisRecord(search);
        if(checkIfExists){
        List<Promo> listOfPro = promoFacade.findthisRecord(search);
        Promo[] arrayOfPro = listOfPro.toArray(new Promo[0]);
        sb1.setPromoData(arrayOfPro);
        }
        else{
